...
class SpacetimeConvolution(nn.Module):
    ...
    def __init__(self, in_channels: int, out_channels: int,
        ...
        # Ensure all are 3er-Tupel oder int
        def to_3tuple(val):
            ...
            raise ValueError(f"Invalid kernel/stride/padding/dilation: {val}")

        self.kernel_size = to_3tuple(kernel_size)
        self.stride = to_3tuple(stride)
        self.padding = to_3tuple(padding) if not isinstance(padding, str) else padding
